...
function createHamburger(options) {
    const hamburger = {
        ...
        patties: options.patties || "beef",
        pattyCount: options.pattyCount || 1,
        extras: options.extras || [],
        description: function () {
            const garnishStr = this.garnishes.join(", ");
            const cheeseStr = this.cheeses.join(", ");
            const sauceStr = this.sauces.join(", ");
            const extraStr = this.extras.join(", ");
            const pattyText = this.pattyCount > 1 ? `${this.pattyCount} ${this.patties} patties` : this.patties;
            return `A ${pattyText} hamburger on ${this.buns} bun with garnishes: ${garnishStr}, cheeses: ${cheeseStr}, sauces: ${sauceStr}, and extras: ${extraStr}. Enjoy!`;
        },
    };
    return hamburger;
}
